...
def find_single_xor(encoded_string):
    ...
    key = max(iter(xor_dict.items()), key=operator.itemgetter(1))[0]
    return chr(key)


def decrypt_sk_xor_message(encoded_string, key):
    unhexed_string = binascii.unhexlify(encoded_string)
    char_list = [chr(char ^ ord(key)) for char in unhexed_string]
    return ''.join(char_list)

KEY = find_single_xor("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736")
MESSAGE = decrypt_sk_xor_message("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736", KEY)
print("Key: " + KEY + "\nMessage: " + MESSAGE)
